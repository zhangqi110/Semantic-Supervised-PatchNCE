...
def define_F(input_nc, netF, norm='batch', use_dropout=False, init_type='normal', init_gain=0.02, no_antialias=False, gpu_ids=[], opt=None):
    ...
    elif netF == 'mlp_sample_m':
        net = PatchSampleF_multipatch(use_mlp=True, init_type=init_type, init_gain=init_gain, gpu_ids=gpu_ids, nc=opt.netF_nc, opt=opt)
...
class PatchSampleF(nn.Module):
    ...
    def forward(self, feats, num_patches=64, patch_ids=None, return_surrund=False):
        return_ids = []
        return_feats = []
        if return_surrund:
            return_surrund_patchids = []
        ...
        for feat_id, feat in enumerate(feats):
            ...
            if num_patches > 0:
                ...
                if return_surrund:
                    surrund_patchids = []
                    dx = [-1, 1, -W, W, -W+1, -W-1, W+1, W-1]
                    for i in range(len(dx)):
                        patch_id_temp = patch_id + dx[i]
                        surrund_patchids.append(patch_id_temp.unsqueeze(0))
                    
                    surrund_patchids = torch.cat(surrund_patchids, dim=0).transpose(0, 1)
                    surrund_patchids[surrund_patchids < 0] = -1
                    surrund_patchids[surrund_patchids >= H*W] = -1
                    # ii = torch.div(patch_id , H, rounding_mode='floor')
                    # jj = patch_id % W
                    # dx, dy = [0, -1, 1], [0, 1, -1]
                    ...
                    #                     surrund_patchid.append((x * H + y).item())
                    #                 else:
                    #                     surrund_patchid.append(-1)
            ...
            if return_surrund:
                return_surrund_patchids.append(surrund_patchids)
        ...
        if return_surrund:
            return return_feats, return_ids, return_surrund_patchids
        else:
            return return_feats, return_ids
    
class PatchSampleF_multipatch(nn.Module):
    def __init__(self, use_mlp=False, init_type='normal', init_gain=0.02, nc=256, gpu_ids=[], opt=None):
        # potential issues: currently, we use the same patch_ids for multiple images in the batch
        super(PatchSampleF_multipatch, self).__init__()
        self.l2norm = Normalize(2)
        self.use_mlp = use_mlp
        self.nc = nc  # hard-coded
        self.mlp_init = False
